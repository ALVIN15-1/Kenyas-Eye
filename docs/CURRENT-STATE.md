...
- **EEA fallback for the photoreal tileset.** `createGooglePhotorealistic3DTileset` returns 403 for any Google Cloud project on an EEA billing account created on or after 8 July 2025, however correctly the key is configured: the restriction follows the billing address, not the requester, so no key setting recovers it. When that direct call throws, `src/main.js` retries the same imagery through Cesium ion asset `2275207`, Cesium's own resale of the tileset under a non-EEA Google contract, and only falls back to the Cesium globe if that also fails. Needs `CESIUM_ION_TOKEN`; without one the previous globe fallback is unchanged.
- **Cesium OSM Buildings** (ion asset `96188`, ~350M OpenStreetMap-derived extrusions) loads at init when an ion token is present and is owned by `src/main.js`. `MapStackController` only toggles its `show`, mirroring `googleTileset`: hidden on `photoreal` (the photogrammetry already carries buildings, and drawing both puts grey boxes inside the real ones), shown on every globe stack, where an aerial basemap would otherwise be flat. Failure to load is cosmetic and logged, never fatal.
