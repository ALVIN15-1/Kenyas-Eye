...
- Optional-key precedence in `dev-fresh.sh` is uniform — explicit shell env, then `.env`, then Keychain: `OPENAI_API_KEY` (Keychain `openai-api`/`api-key` — voice + HUD summary), `AISSTREAM_API_KEY` (`aisstream-api`/`api-key` — live vessels), `CESIUM_ION_TOKEN` (`cesium-ion`/`token`, then `cesium-ion`/`api-key` — Bing stacks), `TOMTOM_API_KEY` (`tomtom-api`/`api-key` — live traffic flow), `FIRMS_MAP_KEY` (`firms-map`/`map-key` — live fires), `GOOGLE_SERVER_API_KEY` (`google-server-api`/`api-key`, then `google-geocoding-api`/`api-key` — geocoding, Places, Street View), `LL2_API_TOKEN` (`.env` only)
...
- Every Google web-service call reads `serverGoogleKey()` in `vite.config.js`: `GOOGLE_SERVER_API_KEY`, then `GOOGLE_GEOCODING_API_KEY`, then `GOOGLE_MAPS_API_KEY`. The browser-bundled `GOOGLE_MAPS_API_KEY` must carry an HTTP-referrer restriction, and Google rejects referrer-restricted keys for Geocoding, Places, and Street View, so those APIs need a second, server-only credential. The last fallback preserves single-key setups whose key is unrestricted.
- `/api/google/geocode` brokers forward (`address`) and reverse (`latlng`) geocoding for location search, annotation anchors, and voice radio/reverse lookups. It forwards only `address`, `latlng`, `bounds`, `region`, `language`, and `result_type`, so a caller cannot append its own `key` and turn the proxy into a relay billed elsewhere. It shares the Google rate limiter, returns Google's own error shape on failure, and never echoes the upstream URL, which carries the key. No browser code calls `maps.googleapis.com` directly.
