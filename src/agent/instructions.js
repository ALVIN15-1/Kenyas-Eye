...
// The GEV operating manual handed to the model on every request.
//
// These 50 directives are the accumulated, tuned behaviour of the product: the
// counting contract, the disambiguation tables, the annotation policy, the
// refusal semantics. They are shared VERBATIM between the voice transport and
// the text transport, because they encode what the app does, not how the user
// reached it.
//
// Only the handful of directives that describe the CHANNEL differ per modality,
// and those are overridden by index below rather than by maintaining a second
// copy of the manual. Editing a shared line changes both transports on purpose.
//
// Serialized weight is roughly 4,700 tokens; with the 28 tool schemas it forms
// the ~11,300-token prefix that MIN_TOOL_CONTEXT_TOKENS exists to protect.

/** Directives common to every transport, in priority order. */
const GEV_SHARED_DIRECTIVES = Object.freeze([
...
/**
 * Directives replaced when the agent answers in text rather than speech.
 *
 * Keyed by index into GEV_SHARED_DIRECTIVES. Kept deliberately small: every
 * override is a line that would otherwise instruct a text agent to "speak",
 * which produces stage directions in the transcript instead of an answer.
 */
const TEXT_DIRECTIVE_OVERRIDES = Object.freeze({
  0: "You are GEV Command, a concise text controller for a Cesium geospatial app called God's Eye View.",
  1: 'Reply in short written messages. The user types commands into a console panel; there is no microphone and no spoken audio.',
  24: 'When a request requires a tool call, call the tool first and write nothing alongside the call.',
  26: 'After receiving tool output, write exactly one short confirmation. Do not repeat the confirmation.',
  43: 'Keep written confirmations to one short line, e.g. "Opening datacenters" or "Flying to London".',
});

/** Modalities the instruction builder understands. */
export const GEV_MODALITIES = Object.freeze(['voice', 'text']);

/** Modality assumed when none is supplied. */
export const DEFAULT_GEV_MODALITY = 'voice';

/** Whether a value names a supported modality. */
export function isKnownModality(modality) {
  return typeof modality === 'string' && GEV_MODALITIES.includes(modality);
}

/**
 * Build the instruction directives for one modality.
 *
 * @param {{modality?: 'voice'|'text'}} [options]
 * @returns {string[]} The directive list, shared lines with channel overrides applied.
 */
export function gevDirectives({ modality = DEFAULT_GEV_MODALITY } = {}) {
  const resolved = isKnownModality(modality) ? modality : DEFAULT_GEV_MODALITY;
  if (resolved === 'voice') return [...GEV_SHARED_DIRECTIVES];
  return GEV_SHARED_DIRECTIVES.map(
    (line, index) => (Object.hasOwn(TEXT_DIRECTIVE_OVERRIDES, index) ? TEXT_DIRECTIVE_OVERRIDES[index] : line),
  );
}

/**
 * Build the newline-joined instruction string sent as the system prompt.
 *
 * @param {{modality?: 'voice'|'text'}} [options]
 * @returns {string}
 */
export function buildGevInstructions(options = {}) {
  return gevDirectives(options).join('\n');
}

/** Directive count, exported so tests can pin that no line is silently dropped. */
export const GEV_DIRECTIVE_COUNT = GEV_SHARED_DIRECTIVES.length;
