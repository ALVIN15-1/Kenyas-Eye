...
- **ALPR / Flock cameras.** Queries the generic `/api/overpass` proxy for `man_made=surveillance` + `surveillance:type=ALPR` nodes. Queries use a quantized ~0.14° box around the camera (or the known-good Lannon / US-45 bbox `43.10,-88.28,43.24,-88.08` when that corridor is in view) so public mirrors and the disk cache can answer instead of 502ing on wide metro dumps. Capped at 600 results per query. When Overpass is unavailable, a bundled ODbL snapshot of mapped nodes for that home corridor is shown as stale fallback. This is mapped, community-tagged camera *locations and public metadata only* — never plate records, vendor accounts, or private Flock/OSM data. Operator/manufacturer tags containing "Flock" render distinctly from other tagged ALPR vendors; a `camera:direction`/`direction` tag draws a short facing-direction line. Coverage reflects what DeFlock/OSM contributors have mapped, not a complete survey.
